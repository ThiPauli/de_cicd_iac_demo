...
def test_dag_id_matches_file_name(dag_file):
    ...
    assert dag_id in dag_bag.dags, f"DAG ID '{dag_id}' not found in DagBag for file '{dag_file}'"

    dag = dag_bag.get_dag(dag_id)
    assert dag.dag_id == dag_id, f"DAG ID '{dag.dag_id}' does not match file name '{dag_file}'"
...
def test_no_import_errors():
    assert len(dag_bag.import_errors) == 0, f"Total import failures: {len(dag_bag.import_errors)}"
